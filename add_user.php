<?php
  ...
  if(isset($_POST["user_data_submit"])){
	...
	if (isset($_POST["email"]) and !empty($_POST["email"])){
		...
		$email = filter_var($email, FILTER_VALIDATE_EMAIL);  //Valideerib emaili
		if ($email === false) {
	...
	function email_registered($email) {
		...
		$stmt = $conn -> prepare("SELECT vr21_users_email FROM vr21_users");
	...
	if (email_registered($email) == 1){
		$email_error = "Sisestatud e-postiaadress on juba kasutusel!";
	}
    //Kontrolli lõpp.
    //parooli kontroll
    ...
    if(empty($_POST["confirmpassword_input"])){
        $confirm_password_error = "Korrake salasõna!";
    } else {
        if($_POST["confirmpassword_input"] != $_POST["password_input"]) {
            $confirm_password_error = "Salasõnad ei ühildu!";
        }
    }
    //Kui vigu ei ole siis salvestame.
