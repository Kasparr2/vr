...
function sign_up($name, $surname, $gender, $birth_date, $email, $password){
    ...
    $stmt = $conn -> prepare("INSERT INTO vr21_users (vr21_users_firstname, vr21_users_lastname, vr21_users_birthdate, vr21_users_gender, vr21_users_email, vr21_users_password) VALUES (?,?,?,?,?,?)"); //Nii mitu välja, nii mitu ?
    echo $conn -> error;
    //krüpteerime parooli
    $options = ["cost" => 12];
    $password_hash = password_hash($password, PASSWORD_BCRYPT, $options);

    $stmt -> bind_param("sssiss", $name, $surname, $birth_date, $gender, $email, $password_hash);
    ...
    function sign_in($email, $password){
        $notice = 0;
        $conn = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]); //loome ühenduse andmebaasiga
        ...
        if($stmt -> fetch()){
            ...
            if(password_verify($password, $password_from_db)){
                ...
                $notice = 1; //Kui õnnestub sisse logida paneme asjad kinni
